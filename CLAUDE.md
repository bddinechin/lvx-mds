...
- **`lvx-family/`** — the architecture description for the LVX family: YAML source files, per-core entry points, and family-specific back-end overrides (TEX docs, GBU header). **Sources only** — see `lvx-refs/` below for generated content.
- **`lvx-refs/`** — committed regression-test baselines: the generated output every backend's `refs`/`diff`/`check` targets compare against, mirroring `lvx-family/`'s `BE/<backend>/`, `MDD/`, and `FE/YAML/lvx/<core>/` layout. This used to live inside `lvx-family/` itself (mixed in with the genuine hand-authored sources); it was split out into its own top-level directory to keep `lvx-family/` down to just sources — see "Reference workflow" below for the full rationale and the `REFDIR` variable that points here.
...
| Target | Effect |
|---|---|
| `make config` | Runs the configure line above into `lvx-mds/build_lvx/`, with `--with-binutils-prefix`/`--with-gdb-prefix`/`--with-gcc-prefix`/`--with-newlib-prefix` pointed at the sibling `lvx-binutils`/`lvx-gdb`/`lvx-gcc`/`lvx-newlib` checkouts so `BE/GBU`/`BE/LIBC` (and eventually `BE/GDB`/`BE/GCC`) install straight into them. **Those four and no more** — notably not `--with-gem5-prefix`, so `BE/GEM5`'s install target has no real destination until one is passed by hand |
...
This Makefile isn't part of this repo's own git history — it lives in `lvx-csw/`, which is its own git repository (`github.com/bddinechin/lvx-csw`) holding the `Makefile`, its own `CLAUDE.md` and `lvx-target/`, and carrying the six sibling checkouts (`lvx-mds`, `lvx-binutils`, `lvx-gdb`, `lvx-gcc`, `lvx-newlib`, `lvx-gem5`) as **submodules**. So a change here is landed twice: committed in this repo, then recorded in `lvx-csw` as a submodule bump.
...
| `REFDIR` | Absolute path to the committed regression-reference tree (`--with-ref-path`, defaults to `lvx-refs/`, sibling to `lvx-family/`). Every backend's `refs`/`diff`/`check` target reads/writes here — see "Reference workflow" below. |
...
cd build_lvx
make all
make refs   # copies Opcode.txt/Description.yml and per-backend reference outputs into lvx-refs/
```

Committed reference files (`lvx-refs/FE/YAML/lvx/<core>/{Opcode.txt,Description.yml}`, `lvx-refs/MDD/lvx/*.table`, plus each back-end's own `refs`-copied outputs under `lvx-refs/BE/<backend>/`) serve as the known-good snapshot; `make diff`/`make check` compare the current build's output against them.

**The two halves of `lvx-refs/` are not equally negotiable, and this is the acceptance test for any schema change.** `lvx-refs/MDD/` is an internal representation: it changes whenever the schema does, and regenerating it is routine. `lvx-refs/BE/` is the *product* — 336 files whose content is a published contract. `BE/GBU`'s output in particular defines the `LVX_*_REGFILE_*` macros and the `lvx_*_regfiles[]` arrays that **hand-written** C in `lvx-binutils` and `lvx-gdb` depends on (`gas/config/tc-lvx.{c,h}`, `opcodes/lvx-dis.c`), and none of that fails at compile time when it drifts.

So when refactoring the schema rather than the ISA, the bar is: **every file under `lvx-refs/BE/` byte-identical after a clean rebuild, while `lvx-refs/MDD/` is free to change.** Verify it directly rather than trusting `make check`, which will happily accept a regenerated reference:

```sh
make clean && make config && make all
for be in GBU GCC GDB LAO TEX LIBC; do
  diff -r lvx-refs/BE/$be build_lvx/BE/$be   # must be empty
done
```

That is how the RegFile/RegClass unification was landed: all 336 back-end files identical, only the MDD tables changed.

**`lvx-refs/` used to live inside `lvx-family/`** (e.g. `lvx-family/BE/GBU/lvx/opcodes/lvx-opc.c`), mixed in with the genuine hand-authored sources — architecturally wrong, since `lvx-family/` is supposed to be the pure ISA-description source repo, and it made it easy to mistake a generated regression snapshot for something hand-maintained. Every backend's `Makefile.in` now reads/writes its reference tree via the `REFDIR` variable (`$(REFDIR)/BE/<backend>/...` etc.) instead of `$(FAMDIR)/BE/<backend>/...`, and `REFDIR` defaults to this sibling `lvx-refs/` directory. `lvx-family/` keeps only genuine sources: the hand-authored `*.yml` (plus their own `*.pl` helper scripts, `Makefile.in`, `scripts/`), and the hand-maintained `BE/GBU/lvx_elfids.h`.

One subtlety this surfaced, since fixed: **`BE/TEX` used to not only *compare against* its reference tree, it also *consumed* it as a build input.** `TEX_coredirs_src` (in `MDS/BE/TEX/Makefile.in`) copied the previous reference tree's per-core directory wholesale into the build output *after* generation — apparently to carry forward hand-authored per-register `.tex` prose, mirroring a genuine pattern from KVX's `BE/TEX` (a handful of `kv3_v1/kv3_v2/Register-*.tex` files still sitting in `lvx-refs/BE/TEX/lvx/` are KVX leftovers from that era). But for the actual LVX family, every single `.tex` file under `lvx-refs/BE/TEX/lvx/{lvx_v1,lvx_v2}/` — including the per-register ones — corresponds directly to one of `MDS/BE/TEX/BIN/{Register,RegFile,DwarfId,Format,...}.pl`; there's no genuine hand-authored LVX content there at all. Since that `cp -rf` was a plain merge, it silently overwrote any freshly-regenerated file that happened to share a name with something already in the reference — masking generator/reference drift for those files completely. This let the `lvx_v1` reference drift for a long time without anyone noticing (`make check` always trivially passed, since it was really just comparing the reference against a copy of itself): by the time this was caught, **388 of 821 documented mnemonics in the `lvx_v1` TEX reference no longer existed in the current ISA** (e.g. `abdbo`, dropped along with the `RegFile-CSR` register file at some point). Removed the copy-forward step entirely and regenerated the `lvx_v1`/`lvx_v2` reference from scratch to match the current ISA — `BE/TEX`'s `check` now genuinely validates its generators against reality. If you ever need to add a *new* backend with its own `refs`/`diff`/`check` targets, search for every `$(ARCHDIR)`/`$(FAMDIR)` occurrence in its `Makefile.in` (not just the obvious ones) before assuming a reference-tree move is complete — this exact class of miss broke the whole build once already, silently stopping the top-level `make all` right after `BE/TEX` with no other backend ever running.
